...
public static class ScenarioSchema
{
    ...
    [
        ...
        new("endsWithLabel", false, Taking.Text, "the key whose declared string the reading should end with, for a state an application appends rather than announces in front — the same precision as the prefix and for the same reason, since the name before it is free text that can hold the word", [], Claims: true),
        new("notEndsWithLabel", false, Taking.Text, "the key whose declared string the reading must NOT end with, for a state an application appends and must not be showing — the mirror of 'endsWithLabel', because a mark that can only be claimed present is one a window drawing it always would pass", [], Claims: true),
