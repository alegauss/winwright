...
public sealed class UnearnedGreenException : Exception
{
    ...
    public UnearnedGreenException(string unearned, IReadOnlyList<string> notRun, IReadOnlyList<string> failed)
        : base($"'{unearned}' is not this run: {Because(notRun, failed)}")
    {
        Unearned = unearned;
        NotRun = notRun;
        Failed = failed;
    ...
    private static string Because(IReadOnlyList<string> notRun, IReadOnlyList<string> failed)
    {
