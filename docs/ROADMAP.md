...
- ⏳ **WW294** (deps: —) **an expectation whose value the application reports can only be a set, and the submenu's marks are single values** — claude-tray still answers those eight facts only as prose in `--profiles`, so nothing there can name one yet. → §WW294
