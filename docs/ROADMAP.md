...
- 📋 **WW273** (deps: —) **a locator can only name an element by a string typed into the case** — The sidebar items are bare Borders with no automation peer, so the words are the only thing that addresses them — and a word typed here is wrong in every other language. → §WW273
- 📋 **WW274** (deps: —) **a locator step names one control type, so a rule governing a family of controls cannot be swept** — The naming rule governs every ComboBox, Slider, TextBox and switch on a panel, and a sweep of them is four steps that are each mostly a hole. → §WW274
...
- ⏳ **WW84** (deps: WW273, WW274) **the names case observes what no screenshot can** — The walk over all six panels and the sweep over every contentless control need a locator built out of a declared string and one naming several types. → §WW84
