...
- 📋 **WW270** (deps: —) **a case can claim a reading is one declared string and never that it is not one** — The live headline reading the app's own unavailable label means the switch left the strip watching nothing, and `never` is about a window over a wait rather than one reading at rest. → §WW270
