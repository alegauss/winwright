...
- 📋 **WW297** (deps: —) **the roll call never runs on a failing suite, so the worst runs report no excuses and file no ledger** — Measured on a red guest run: the target is AfterTargets=VSTest, which MSBuild skips when VSTest fails, so a red run reports no excuses and files no ledger. → §WW297
