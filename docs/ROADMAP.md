...
- 📋 **WW236** (deps: —) **a case cannot check a derived set, so the criterion about them cannot be tried** — The engine derives a set from the project's strings and a data file has no name for one, which makes the block's first criterion unfalsifiable rather than met. → §WW236
