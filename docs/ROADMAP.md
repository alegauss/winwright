...
- 📋 **WW181** (deps: —) **the ghost census says none where the flyout would not open, so it reports a clean desk it never looked at** — The reading says what it could not see, because a sentence claiming none on a shell that refused to open is a green covering what never ran. → §WW181

## Block C — Locate — the locator grammar and the tree an agent reads

- 📋 **WW175** (deps: —) **a deadline handed a look that never answers null returns on the first look, and nothing says the wait was skipped** — A wait on a look that cannot answer nothing is refused rather than quietly met, because a deadline that never waited reads exactly like one satisfied at once. → §WW175

## Block D — Act — patterns before pointers

- 📋 **WW174** (deps: —) **a tray menu that never came up records Failed whether the code or the desk was at fault, and answers no verdict** — The menu carries the three-way answer its own search now has, because recording Failed for a desk that refused is the collapse WW168 closed one call further down. → §WW174
...
- 📋 **WW178** (deps: —) **the fixture is claimed to need no account, no network and no second display, and nothing in the suite checks any of it** — The claim is read back off the built fixture, because a block is called finished on this criterion and the only thing holding it up is whoever last read it. → §WW178
