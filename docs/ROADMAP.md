...
- 📋 **WW325** (deps: —) **no reading answers whether a control will take input, so a case cannot check that a button is greyed** — Disabled is the state a half-finished form is supposed to be in, and it is the one an application gets wrong by enabling too early. → §WW325
