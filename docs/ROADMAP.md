...
- ⏳ **WW294** (deps: WW295) **an expectation whose value the application reports can only be a set, and the submenu's marks are single values** — claude-tray still answers those eight facts only as prose in `--profiles`, so nothing there can name one yet. → §WW294
- 📋 **WW295** (deps: —) **a locator can be built out of a declared string but never out of a value the application reports** — `{key}` resolves through the strings, so a case cannot select the entry for the profile the app says its icon follows — the claim the check mark is about. → §WW295
