...
- 📋 **WW311** (deps: WW305 ✅) **a run that just booted the guest is measured on a desk still putting up its startup notifications** — It has stopped being noise in a count: the toast held the foreground while the adoption's keyboard case ran, and that case came back unchecked with three steps unwalked. → §WW311
- 📋 **WW314** (deps: WW305 ✅) **a guest sitting at its lock screen is found only after the tree is carried, so the refusal costs a full sync** — The session is checked where the suite starts, and WW305 made cold starts cheap enough to be common — which is exactly when a guest is most likely to be locked. → §WW314
- ⏳ **WW315** (deps: —) **six of the eleven adopted cases have nothing to check on the guest, which holds no profiles and no transcripts** — Each is a hole naming the file it wanted, which is right — but the cases WW83 and WW85 still have to migrate could then be proven nowhere. → §WW315
...
- **two profiles and a transcript exist on the guest, as disposable fixture** The six
  holed cases answer instead of naming an absent file, and everything the fixture wrote
  is deleted with the rest of the tree — no profile a run repointed surviving outside
  it. That is what lets WW83 and WW85 migrate as cases observed to work, rather than
  correct and refused.
