...
- 📋 **WW193** (deps: —) **two source-read catalogues carry the same tree walk and a third is asked for, and the first got its exclusions wrong** — One reading walks the sources, because the exclusion this got wrong once cost two guest runs and a third copy is a third chance to get it wrong. → §WW193
