...
public class VerdictTests
{
    ...
    {
        Assert.Throws<ArgumentException>(() => AssertionResult.Unchecked("  ", "no profile registered"));
    }

    [Fact]
    public void A_hole_with_no_reason_is_refused()
    {
        Assert.Throws<ArgumentException>(() => AssertionResult.Unchecked("the report renders", ""));
    }
}
