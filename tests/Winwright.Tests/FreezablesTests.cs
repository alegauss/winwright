...
public sealed class FreezablesTests
{
    ...
    {
        var shared = OnStaThread(() => Freezables.Shared(new SolidColorBrush(Colors.Red)));
    ...
    {
        var (sharedIsFrozen, theirsIsFrozen) = OnStaThread(() =>
        {
    ...
    {
        var theirs = OnStaThread(() => new SolidColorBrush(Colors.Red));
    ...
    public void A_brush_that_cannot_be_frozen_stays_bound_however_it_is_passed_around()
    {
        ...
        var refused = Assert.Throws<ThreadBoundException>(
            () => OnStaThread(() => Freezables.Shared(new VisualBrush(new Border { Width = 4, Height = 4 }))));
    ...
    public void Shareable_answers_null_rather_than_refusing_where_a_caller_has_a_fallback()
    {
        ...
        Assert.NotNull(Freezables.Shareable(OnStaThread(() => Freezables.Shared(new SolidColorBrush(Colors.Red)))));
    }
    ...
    public void A_render_handed_a_brush_from_another_thread_is_refused_before_it_draws()
    {
        ...
        var made = OnStaThread(() => new SolidColorBrush(Colors.Blue));

        var refused = Assert.Throws<ThreadBoundException>(
            () => OnStaThread(() => Render.ToBitmap(new Border { Width = 10, Height = 10 }, background: made)));
    ...
    public void A_render_handed_an_element_from_another_thread_is_refused_the_same_way()
    {
        ...
        var refused = Assert.Throws<ThreadBoundException>(() => OnStaThread(() => Render.ToBitmap(made)));
    ...
    {
        var shared = OnStaThread(() => Freezables.Shared(new SolidColorBrush(Colors.Blue)));
